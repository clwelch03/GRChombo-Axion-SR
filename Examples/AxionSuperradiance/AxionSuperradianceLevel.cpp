...
#include "AxionSuperradianceLevel.hpp"
...
void AxionSuperradianceLevel::specificAdvance()
{
...
void AxionSuperradianceLevel::initialData()
{
    CH_TIME("ScalarFieldLevel::initialData");
    if (m_verbosity)
        pout() << "ScalarFieldLevel::initialData " << m_level << endl;

    // Set everything to zero, then lay a Kerr BH metric and the axion cloud on
    // top. KerrBH fills only the metric vars (chi, h_ij, K, lapse, shift);
    // InitialAxionData fills only the scalar field (phi, Pi). They write
    // disjoint variables, so they can run in the same compute pack.
    InitialAxionData initial_data(m_p.initial_params, m_dx);
    BoxLoops::loop(make_compute_pack(SetValue(0.),
                                     KerrBH(m_p.kerr_params, m_dx), initial_data),
                   m_state_new, m_state_new, INCLUDE_GHOST_CELLS);
...
void AxionSuperradianceLevel::postRestart()
{
...
void AxionSuperradianceLevel::prePlotLevel()
{
    fillAllGhosts();
    Potential potential(m_p.potential_params);
    ScalarFieldWithPotential scalar_field(potential);
...
void AxionSuperradianceLevel::specificEvalRHS(GRLevelData &a_soln, GRLevelData &a_rhs,
                                       const double a_time)
{
    // Enforce trace free A_ij and positive chi and alpha
    BoxLoops::loop(
...
void AxionSuperradianceLevel::specificUpdateODE(GRLevelData &a_soln,
...
void AxionSuperradianceLevel::specificPostTimeStep()
{
#ifdef USE_AHFINDER
